        ...
        Request request = new RetrofitRequest<>(call, new ResponseListener<LoginModel>() {
            ...
            public void onResponse(int code, LoginModel response, Headers headers) {
                ...
                if (response.getResponse().getResponseval()) {
                    ...
                    Log.e("Testloaduser", "" + response.getUserid());
                    Intent intent = new Intent(PaymentCollection.this, CustomerPaymentDetails.class);
                    intent.putExtra("Select_User_ID", userid);
                    startActivity(intent);


                    /*new PreferenceManager(Loaduser.this).setLoginModel(response);
                    new PreferenceManager(Loaduser.this).setAutoLogin(true);*/
                  //  startActivity(new Intent(PaymentCollection.this, CustomerPaymentDetails.class));
                   // Toast.makeText(PaymentCollection.this, "Payment Collection", Toast.LENGTH_SHORT).show();


                }

                else {
                    Toast.makeText(PaymentCollection.this, "" + response.getResponse().getReason(), Toast.LENGTH_SHORT).show();
                }
            }
